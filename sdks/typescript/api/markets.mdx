...
<ParamField path="query" type="string">
  Search markets by title, subtitle, ticker, or category. Results are sorted by match relevance (title matches first, then subtitle, ticker, category).
</ParamField>
...
```typescript
// Search for markets mentioning "Price" in title, subtitle, or ticker
const response = await client.getMarkets({
  query: 'Price',
  status: 'active'
});

console.log(`Found ${response.markets.length} markets matching "Price"`);
