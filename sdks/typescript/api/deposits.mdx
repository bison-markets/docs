...
<ParamField path="amountUusdc" type="bigint" required>
  Amount in µUSDC (e.g., `100000000n` for 100 USDC). Use `usdcToUusdc()` helper for conversion.
</ParamField>
...
const txHash = await client.executeDepositFlow({
  ...
  chain: 'base',
  amountUusdc: usdcToUusdc(100), // Helper converts 100 USDC to 100000000n µUSDC
});
...
<ParamField path="amountUusdc" type="bigint" required>
  Amount in µUSDC (e.g., `50000000n` for 50 USDC). Must not exceed your deposited balance. Use `usdcToUusdc()` helper for conversion.
</ParamField>
...
const withdrawal = await client.scheduleWithdraw({
  ...
  userAddress: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
  chain: 'base',
  amountUusdc: usdcToUusdc(50), // 50 USDC
});
...
try {
  const txHash = await client.executeDepositFlow({
    ...
    userAddress: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
    chain: 'base',
    amountUusdc: usdcToUusdc(1000), // 1000 USDC
  });
...
- Use `usdcToUusdc()` to convert USDC to µUSDC (returns bigint)
