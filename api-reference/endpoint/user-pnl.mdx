...
<ResponseField name="totalDeposits" type="number" required>
  Total USDC deposited in µUSDC. Excluded when filtering by `marketId`.
</ResponseField>

<ResponseField name="totalWithdrawals" type="number" required>
  Total USDC withdrawn (claimed) in µUSDC. Excluded when filtering by `marketId`.
</ResponseField>

<ResponseField name="realizedPnl" type="number" required>
  Realized profit/loss in µUSDC. Positive = profit, negative = loss.
</ResponseField>

<ResponseField name="unrealizedPnl" type="number" required>
  Estimated unrealized profit/loss in µUSDC based on open positions.
</ResponseField>

<ResponseField name="netPnl" type="number" required>
  Total PNL (realized + unrealized) in µUSDC.
</ResponseField>

<ResponseField name="totalFeesPaid" type="number" required>
...
<ResponseField name="timeSeries" type="array">
  ...
  <Expandable>
    ...
    <ResponseField name="cumulativePnl" type="number">Cumulative PNL at this point in µUSDC</ResponseField>
  </Expandable>
</ResponseField>
